...
from duplicateAndDeletedFileTracker.main import (
    CursorInterface, ExtendedCursorInterface, getDuplicateManagementCallbacks,
...
class archiveDatabaseTestCase(unittest.TestCase):
    def setup_db_for_test(self, cursor: CursorInterface):
        ...
        shutil.copytree(config.fileStructurePath,
                        config.rootPath, dirs_exist_ok=True)
    ...
    def test_createTable_and_persistentInsert(self):
        with openConnection(config.connect) as cursor:
            ...
            cursor.execute(
                "INSERT INTO currentFiles (relative_path) VALUES ('test')")
    ...
    def test_loadCurrentFiles(self):
        with openConnection(config.connect) as cursor:
            ...
            result = cursor.getResult(
                "SELECT relative_path, modified FROM currentFiles")
            for element in [
                ...
                ('foxtrot\\present_hasDup.txt',
                 datetime.datetime(2022, 7, 12, 7, 19, 53))
            ]:
    ...
    def test_update_newPathFiles_and_modifiedFiles_hashes(self):
        with openConnection(config.connect) as cursor:
            ...
            result = cursor.getResult(
                "SELECT relative_path, file_hash FROM currentFiles")
            for element in [
                ('alpha\\new_isDup.txt',
                 '740ad0f2a20c5d4167f7a299aaa044ffddd1ea82a290cfbdcf0eefb27da342d5'),
    ...
    def test_updateArchiveMovedFiles(self):
        with openConnection(config.connect) as cursor:
            ...
            self.assertPathInTable(
                cursor, 'foxtrot\\movedFile.txt', "archiveFiles")
            self.assertPathNotInTable(
                cursor, 'alpha\\movedFile.txt', "archiveFiles")
            self.assertPathInTable(
                cursor, 'foxtrot\\movedWithNewAtOrginalLoc.txt', "archiveFiles")
            self.assertPathNotInTable(
                cursor, 'alpha\\bravo\\moved-newOrginalLoc.txt', "archiveFiles")
            ...
            self.assertPathInTable(
                cursor, "alpha\\bravo\\moved-newOrginalLoc.txt", "newUnseenFiles")
            self.assertPathNotInTable(
                cursor, "alpha\\bravo\\moved-newOrginalLoc.txt", "modifiedFiles")
    ...
    def test_updateArchiveModifiedFiles(self):
        with openConnection(config.connect) as cursor:
            ...
            result = cursor.getResult(
                "SELECT relative_path, file_hash, modified FROM archiveFiles")
            self.assertIn(
                ('alpha\\bravo\\modified.txt',
                    'e8ce5dcaf408935ff76747226d2e8bee4319a2f593c1d7a838115e56183d1f37',
    ...
    def test_updateArchiveDeletedFiles(self):
        with openConnection(config.connect) as cursor:
            ...
            self.assertPathNotInTable(
                cursor, "alpha\\deletedFile.txt", "archiveFiles")
            self.assertPathInTable(
                cursor, "alpha\\deletedFile.txt", "archiveDeletedFiles")
    ...
    def test_removeAllDuplicates(self):
        with openConnection(config.connect) as cursor:
            ...
            self.assertPathNotInTable(
                cursor, "alpha\\new_isDup.txt", "archiveFiles")
            self.assertFalse(
                Path(config.rootPath, "alpha\\new_isDup.txt").exists())

    def test_removeDuplicateById(self):
        with openConnection(config.connect) as cursor:
            self.setup_with_hash_reading(cursor)

            r_id = getDuplicateManagementCallbacks(
                cursor, "duplicateFiles", config.rootPath)["r###"]
            file_id = cursor.getResult(
                "SELECT file_id FROM duplicateFiles WHERE relative_path = 'alpha\\new_isDup.txt'"
            )[0][0]
            r_id(file_id)
    ...
    def test_removeAllDuplicatesPreviouslyDelected(self):
        with openConnection(config.connect) as cursor:
            ...
            self.assertPathNotInTable(
                cursor, "foxtrot\\dupPreviouslyDeleted.txt", "archiveFiles")
            self.assertFalse(
                Path(config.rootPath, "foxtrot\\dupPreviouslyDeleted.txt").exists())
